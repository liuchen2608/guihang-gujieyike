...
export default function HomeClient() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void loadHomeSession(window.localStorage, (input) => fetch(input, { cache: "no-store" }))
        .then((session) => {
          if (cancelled) return;
  ...
  return (
    <main ref={pageRef} className="landing-page" onPointerMove={moveAtmosphere}>
      ...
      <header className="site-header">
        <div className="brand"><span className="brand-mark"><span>归</span></span><span><strong>归航</strong><small>蛊界异客</small></span></div>
